...
Player 를 나타낼 때 % 쓰는것  
게임판을 배열을 안써도 됐고, Write로 해도 됐다. 1~9부분에 새로운 배열을 사용하였고 그렇게하면 유저가 입력한 값이랑 배열의 위치 값이랑 숫자가 같아져서 더 효율적이고 간단하게 코딩이 가능하다고 생각한다.  많은부분이 필요가 없다.  왜 새로운 배열을 추가한다는 생각을 못 한 걸까?.  
풀이를 보고 생각이 든 게 풀이를 먼저 보고 풀어보는 게 나에게 도움이 될까? 처음부터 해보고 풀이를 보는 게 느끼는 게 더 많을까? 고민을 해봐야 될 부분이다.
{: .notice--success}
...
이번 틱택톡을 하고 풀이를 본 후 느낀 게 많다. 많이 부족할 걸 느꼈고, 어떻게 하면 더 코드를 잘 짤 수 있을까? 그냥 내 생각을 그대로 코드로 만들고 풀이를 보는 게 나에게 도움이 되는가? 아니면 구글링해서 다른 걸 보고 하는 게 나에게 도움이 되는가? 전자는 너무 시간 낭비가 아닐까? 방법이 잘못된 걸까? 생각이 많은 날이었다.
{: .notice}
