...
**풀이를 보고나서**  
Player 를 나타낼 때 나는 if문으로 1,2을 바꿨고 풀이는 %2로 하고 ++하는 방법을 했고, 좋은것 같다.  
1~9부분에 새로운 배열을 사용하였고 그렇게하면 유저가 입력한 값이랑 배열의 위치 값이랑 숫자가 같아져서 더 효율적이고 간단하게 코딩이 가능하다고 생각한다.  map[2,2]~map[10,10] 부분이 필요가 없다.  왜 새로운 배열을 추가한다는 생각을 못 한 걸까?.  
{: .notice--success}
