...
✔ [SerializeField]로 드래그앤드랍, GetComponentInChildren 사용가능  
...
## JoyStick - Player 연결
✔ 3가지 방법  
✔ Find로 PlayerContorller찾기 -> 사용 X
✔ Managers (Static) -> 매 프레임마다 통신하기 힘듬
✔ 델리게이트, 콜백   
{:style="border:1px solid #EAEAEA; border-radius: 7px;"}
{: .notice--info}  
...
**3. 델리게이트 콜백 (4강내용)**  
델리게이트 - 데이터가 바꼈을때 구독한 곳으로 알려준다.  
✔ 조이스틱으로 Managers.Game.MoveDir가 변경된다.  
✔ set => OnMoveDirChanged  
✔ OnMoveDirChanged구독되어 있는 곳들에게 알려준다.  
